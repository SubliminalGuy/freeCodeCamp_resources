...
echo "$($PSQL "SELECT COUNT(*) AS more_than_to_goals FROM games WHERE winner_goals >2;")"
...
echo "$($PSQL "SELECT CONCAT(year, '|', name) FROM games LEFT JOIN teams ON games.winner_id = teams.team_id WHERE round = 'Final' ORDER BY year;")"
